...
{                                                                                              
		const char *atb = "0123456789abcdefghijklmnopqrstuvwxyz"; 
		const unsigned int itb[] = {1U, 36U, 1296U, 46656U, 1679616U, 60466176U, 2176782336U};
		int i;
		if (buf_len < 7) return '\0';     
		if (num >= itb[6]) return '\0';
		memset(buf, '0', buf_len);
		buf[6] = '\0';
		for (i = 5; i >= 0; i --) {
				if (num >= itb[i]) {
						buf[5-i] = atb[num / itb[i]];
						num = num % itb[i];
				}
				if (!num) break;
		}
